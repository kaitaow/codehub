...
需要注意低位是Z，从而在用n/table[curLen-1-i]的时候会把低位的值算进来，从而导致后面的计算出错，比如说52对应的数字是AZ，但是计算的时候用52除以26得到2，导致低位的值直接变为0。
因此在计算的时候要特别处理这种情况。
这道题目和171题是相反的题目，给出需要求title以及给出title求序号。相对来说给出title求需要要简单一些因为不需要处理上面的特殊情况。